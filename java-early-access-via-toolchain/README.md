...
This is a Java 27 _Early Access_ project that's built with Java 25. Wait, what does that mean? This is a bit of a facetious
...
- The Gradle process runs on Java 25
    - Gradle is our build tool. As such, we can facetiously say "we use Java 25 to build the project".
- The compilation process runs on Java 27
    - Specifically, the `./gradlew compileJava` task will kick off a Java 27 `javac` process. Gradle refers to this pattern
      of creating a secondary Java process as "forking a Java process".
- The testing process runs on Java 27
    - Specifically, the `./gradlew test` task will fork a Java 27 process to run the JUnit tests.
- The run process uses Java 27
    - Specifically, the `./gradlew run` task will fork a Java 27 process to run the program (The `static void main(String... args)`
      method)
...
work-in-progress versions of Java. These versions of Java are called _Early Access_ versions. For example, in April 2026
the early access versions of Java were Java 27, Leyden, Valhalla and more (see <https://jdk.java.net>), and the latest version of Gradle was
Gradle 9.4.1 (see <https://gradle.org/releases/>). Gradle 9.4.1 does not run on Java 27. So, at the time it was necessary to have a split-version project.

> Friendly reminder: use the official OpenJDK site to stay up-to-date on the latest OpenJDK plans, like [Java 25](https://openjdk.java.net/projects/jdk/25/spec/).
...
1. Pre-requisite: Java 25 and Java 27 early access
2. Make sure that Java 27 is installed in a location known to Gradle
...
4. Run the program:
    ...
    - It will print the following:
      ```text
      Hello World! I am running in Java 27-beta
      ```
